...
from Heuristic_drone import Greedy_WPS,Tabu,Calculate_Re,Get_TL,Greedy_Min,Tabu_min,Calculate_Acc
import sys
#import pickle 
import copy 
...
# task_dic={'df1':[3,0.3],'dh1':[3,0.1],'dw1':[3,0.1],'dw2':[2,0.01],'df2':[3,0.01],'dh2':[2,0.1],
#           'mf':[5,0.5],'mh1':[4,0.5],'mh2':[4,0.3]}
task_dic={'df':[1,0.0005],'ds':[2,0.0003],'dh':[20,0.0005],'dw':[3,0],'mf':[5,0.005],'ms':[5,0.005],'mh':[4,0.005]}
...
class Controller: 
    ...
    def __init__(self,all_num,lay_num,Wap_ty_set=[],report=[],task_dic={},ins_m_dic={},Dis=[],f_num=2,f_up=1,p_floor=3,w_num=32,To=To,T=T_f):
...
while ii<1:
    ...
    c=Controller(all_num,lay_num,Wap_ty,a,task_dic,ins_m_dic,Dis)
    c.Get_WPC()
    c.Gen_Ma()
    for i in range(-1,2):
        for j in [3]:
