...
# for i in range(len(T)):
    ...
    for size in [2]:
        ...
        for p in [0]:
            #if p==2:
            #dy2022_14_5_2_2_0
            f=open(f"./result/result_2021/dy2022_16_{check}_{drone_num}_{size}_{policy}_{p}.txt",'r')
#             if drone_num==7:
#                 if size==2:
...
for pa in range(len(Total_st[ke])): 
    ...
    plt.xticks(range(len(h[show[0]])),checks)
    plt.grid( linestyle='--', linewidth=1)
    plt.ylabel(metrics2[cc])
    #plt.xlabel('Time Limitation for AMT (s)')
    plt.xlabel('Time Period for Checking Perception Table (Minute)')
    plt.legend(legends,prop={'size': 13},ncol=2)
    filename=(f"./result/2022_result/{par}_tasks_{names2[cc]}_{drone_num}.eps")
    plt.savefig(filename,bbox_inches = 'tight')  
    plt.show()
    plt.clf() 
    cc=cc+1  
