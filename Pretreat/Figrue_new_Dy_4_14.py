...
# for i in range(len(T)):
    ...
    w_min_AUC2=defaultdict(dict); w_sample2=defaultdict(dict); w_dect2=defaultdict(dict);
    total_miss=defaultdict(dict)
    for size in [2]:
        for p in [0]:
            #if p==2:
...
#                 f=open(f"./result/dy21_{drone_num}_{size}_{policy}_{p}.txt",'r')
            ...
            Result_miss={}
            cc=list(range(1,2)) # checking nur
            #cc.remove(11)
            ...
            for line in f: 
                ...
                if re.match("avg_run",str(line))!=None:
                    ...
                    if num in cc:
                ...
                elif re.match("sum_state", str(line)) !=None:#_state
                        a=line.split(" ") 
                        #print(f"why",a)
                        method=int(a[2])
                        dur=int(a[-3])
                        ...
                        if num in cc:
                            ...
                            if  w_min_AUC2.get(method)==None:
                                w_min_AUC2[method]={}
                            if  w_min_AUC2.get(method).get(num)==None:
                                w_min_AUC2[method][num]=[]
                            w_min_AUC2[method][num].append(value)       
            ...
            print(f"see total_miss", total_miss)
   ...
   ############################################################################################## for get the average sample
            ...
            cc=0
            for me in [total_miss,w_min_AUC2,w_sample2,w_dect2]:
                h=[];err=[]
                for i in show:
                    #print(f"why",i, me[i])
                    for num in me[i]:
...
#             h=[];err=[]
#             for i in show:
#                 print(f"why",i, w_AUC[i])
#                 a=np.mean(list(w_AUC[i]))
#                 sd=np.std(list(w_AUC[i]))   # get the std 
#                 er=conf_co*(sd/math.sqrt(len(w_AUC[i])))
#                 h.append(a)
#                 err.append(er)
#             print(h)
...
#             plt.xlabel('Methods')
#             filename=(f"./result/2022_result/{par}_com_AUC_{drone_num}.eps")
#             plt.savefig(filename,bbox_inches = 'tight')  
#             plt.show()
#             h=[];err=[]
#             for i in show:
#                 print(f"why", w_dect[i])
#                 a=np.mean(list(w_dect[i]))
#                 sd=np.std(list(w_dect[i]))   # get the std 
#                 er=conf_co*(sd/math.sqrt(len(w_dect[i])))
#                 h.append(a)
#                 err.append(er)
#             print(f"see h",h)
#             plt.bar(range(len(h)),[h[i] for i in range(len(h))],yerr=err,align='center', alpha=0.5, ecolor='black', capsize=10)
#             plt.xticks(range(len(h)),legends)
#             plt.grid( linestyle='--', linewidth=1)
#             plt.ylabel('Weighted Reliability')
#             plt.xlabel('Methods')
#             filename=(f"./result/2022_result/{par}_com_reliability_{drone_num}.eps")
#             plt.savefig(filename,bbox_inches = 'tight')  
#             plt.show()    
#             h=[];err=[]
#             for i in show:
#                 print(f"why", w_sample[i])
#                 a=np.mean(list(w_sample[i]))
#                 sd=np.std(list(w_sample[i]))   # get the std 
#                 er=conf_co*(sd/math.sqrt(len(w_sample[i])))
#                 h.append(a)
#                 err.append(er)
#             print(h)
#             plt.bar(range(len(h)),[h[i] for i in range(len(h))],yerr=err,align='center', alpha=0.5, ecolor='black', capsize=10)
...
#             plt.xlabel('Methods')
#             filename=(f"./result/2022_result/{par}_com_accuracy_{drone_num}.eps")
#             plt.savefig(filename,bbox_inches = 'tight')  
#             plt.show()
# #
            h=[];err=[]
            for i in show:
                #print(f"why", w_min_AUC[i])
                a=np.mean(list(w_min_AUC[i]))
                sd=np.std(list(w_min_AUC[i]))   # get the std 
                er=conf_co*(sd/math.sqrt(len(w_min_AUC[i])))  
                h.append(a)
                err.append(er)
            plt.bar(range(len(h)),[h[i] for i in range(len(h))],yerr=err,align='center', alpha=0.5, ecolor='black', capsize=10,color='royalblue',)
            plt.xticks(range(len(h)),legends)
            plt.grid( linestyle='--', linewidth=1)
            plt.ylabel('Minimum Weighted AUC')
            plt.xlabel('Methods')
            filename=(f"./result/2022_result/{par}_com_min_AUC_{drone_num}.eps")
            plt.savefig(filename,bbox_inches = 'tight')  
            plt.show()
#######################################################################         
        #show=[7,8,3,4,6,0]
...
#     filename=(f"./result/24_result/{par}_com_al_{drone_num}.eps")
#     plt.savefig(filename,bbox_inches = 'tight')  
#     plt.show()
#     plt.clf() 
