...
last_result={}
for pa in range(len(Total_st[ke])): 
...
#             err[i].append(er)
    ...
    for i in show:
        ...
        for k in list(Total_st.keys()):   # different number of drones. 
            ...
            tmp2=[]
            for c in range(len(tmp)):
                if tmp[c]==0 or last_result[k][c]==0:
                    tmp2.append((tmp[c]+0.001)/(last_result[k][c]+0.001))
                else:
                    tmp2.append((tmp[c])/(last_result[k][c]))
            #tmp2=[(tmp[c]+0.001)/(last_result[k][c]+0.001) for c in range(len(tmp))]
            a=np.mean(tmp2)
