...
def Greedy_TSP2(kind,Cu_Wap_ty,M_set,D,task_dic,ins_m_dic,w_in,T_in,T_tm):
    Wap_set=[]
    if kind==-1:
        for i in Cu_Wap_ty:
            Wap_set.append(list(i))
    ...
    if kind==-1:
        ...
        id_set=[]
        for num in [0,1]:
            id_set=id_set+[Wap_set[num][i].id for i in range(len(Wap_set[num]))]
        while True:
            if len(id_set)>0:
                Dis=sorted([(D[w_id][w],w) for w in id_set])
                w_c=Dis[0][1]
...
def Greedy_Min(case,co_s,Wap_set,M_set,D,task_dic,ins_m_dic,w_in,T_in,T_tm,T_total,Cu_Wap_ty):
    ...
    if case==1 or case==4 or case==9 or case==10 or case==17:   # + greed_cov 9 and 10
        #print(f"see if it is here")
        need,rest,to_w_seq=Check_EM_M(ma_set, M_W_dict)
        #print(f" see need ", need)
        la_t=cu_t
...
def Get_Re_Min(case, ac_w_in,min_re, count,min_set,wap,m_set,cu_t,D,w_id,task_dic,ins_m_dic,T_tm,co_s=1):
    ...
    if case==7 or case==8 or case==9 or case==10 or case==13 or case==17:
...
#         print(f"sort", sorted(Prof.items(),key=lambda item: item[1],reverse=True))
#         print(f"choice", sorted(Prof.items(),key=lambda item: item[1],reverse=True)[0][0])
...
#                 Sum_tl=Sum_tl+len(TL[k])
        ...
        if TL.get(k)==None:   # if areas did not be covered, then get the value directly. 
            for j in range(len(task_set)): 
                ...
                t_in=start_set[j]
                end_t=end_set[j]
                dur=min(Time,end_t)-max(t_in,last_duration)
                if t_in<Time:  
                    #check_time=list(np.arange(t_in+sample_rate,min(Time,end_t),sample_rate))
                    check_time=list(np.arange(max(last_duration,t_in),min(Time,end_t),sample_rate))
                    if len(check_time)>0:
                        c_t=list(map(list,zip(check_time,[0]*len(check_time))))
                        ####################
                        #add_t=c_t+[[last_duration,0]]   # add the inital checking time point, 
                        check_p=[cc for cc in c_t if cc[0]>=last_duration]
                        #####################
                        if b>0:
                            detect=[]
                            sample_one=0
                            All_t=sorted(c_t) ## Checking time only is c_t
                            ####################################################################  Need add last_duration
    #                         sum_one,la,last_b=Sump_Integral(b,0,dy,t_in,min(end_t,Time),la_t)
    #                         task_va[si].append(sum_one/dur)
                            AUC_t=sorted([[last_duration,0],[min(end_t,Time),0]])
                            ########AUC_t=sorted(tl+[[last_duration,0]])
                            ############################################
                            #### based on the modified TL, get the value  
                            sum_one=0        
                            for i in range(len(AUC_t)):
                                acc=ins_m_dic.get(str(int(AUC_t[i][1]))).get(task_set[j][:2]) # get the accuracy of the inspection
                                if(i==0): #calcuate the information loss before this visit, here is a little complicated, we need consider the last  visit Time. 
                                    sum_a,la_t,b=Sump_Integral(b, acc, dy, t_in, AUC_t[i][0],la_t)
                                    if t_in>=last_duration:
                                        sum_one=sum_one+sum_a
                                else:
                                    sum_a,la_t,b=Sump_Integral(b, acc, dy, AUC_t[i-1][0], AUC_t[i][0],la_t)
                                    if AUC_t[i-1][0]>=last_duration:
                                        sum_one=sum_one+sum_a
                            task_va[si].append(sum_one/dur)
                            ##########################################################################  
                            ...
                            rat=len([d for d in detect if d >=detect_threshold])/len(check_p)
                            detect_ratio.append(rat)
                            sample_sum=sample_sum+sample_one/len(check_p)
                            Va_set.append(sample_one/len(check_p))
                            target_detect[task_set[j]].append(rat)
                            detect_av[si].append(rat)
                            target_sample[task_set[j]].append(sample_one/len(check_p))
                            sample_av[si].append(sample_one/(len(check_p)))   
                        else: 
                            #sum_event.append(0)
                            task_va[si].append(0)
            ...
            for j in range(len(task_set)):  # for all tasks of a m 
                ...
                if t_in<Time:
                    end_t=end_set[j]
                    sum_one=0 
                    dur=min(Time,end_t)-max(last_duration,t_in)
                    tl=copy.deepcopy(TL[k])
                    if end_t<Time:
                        tl=[i for i in tl if end_t>=i[0] >=t_in]
                        if len(tl)>0:
                    ...
                    if len(check_time)>0:
                        ...
                        for i in range(len(All_t)):
                            acc=ins_m_dic.get(str(int(All_t[i][1]))).get(task_set[j][:2]) # get the accuracy of the inspection
                            cu_acc,la_t,b=Cur_Integral(b, acc, dy, All_t[i][0],la_t)
                            if All_t[i] in check_p:
                                detect.append(cu_acc)
                                sample_one=sample_one+cu_acc 
                                #print(f"why {cu_acc}")   
                        #print(f"why  {check_time}, {All_t[i]} {t_in}, {last_duration} {end_t}")    
                        sample_sum=sample_sum+sample_one/len(check_p)
                        Va_set.append(sample_one/len(check_p))
                        rat=len([d for d in detect if d >=detect_threshold])/len(check_p)
                        detect_ratio.append(rat)
    ...
    for k, j in task_va.items():
        if len(j)>0:
            w_min_v=w_min_v+int(k)*min(j)
            min_AUC.append(min(j)/int(k))
            w_min_set.append((int(k),min(j)))
    #print(f"why check {sum_event}")
    min_w_AUC=min(min_AUC)
    return event_size,task_va,w_min_v,w_min_set,w_sample_min,w_detect_min,av_w_dect_min,av_w_sample_min,Va_set,sum_w_detect,sum_w_sample,min_detect,min_sample,min_w_AUC
