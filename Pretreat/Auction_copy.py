...
import time      
# class Agent(): 
#     def __init__(self,id,nodeset,M_dis=None,M_fre=None,To=5): 
#         self.id=id
#         self.id_set=nodeset
#         self.V = len(nodeset)
#         self.MT = []   
...
#         self.tsp=[]
#         if len(M_fre)!=0:
#             self.to_t=sum([M_fre[i]*To for i in nodeset])   # travel time plus loitor time
#         self.M_dis=M_dis
# #         if len(M_dis) != 0:
# #             self.graph = [[0 for i in range(len(nodeset))] for j in range(len(nodeset))] 
# #             for i in range(len(nodeset)):
# #                 for j in range(i+1,len(nodeset)):
# #                     #self.graph[i][j]=min(M_fre[nodeset[i]],M_fre[nodeset[j]])*M_dis[nodeset[i]][nodeset[j]]
# #                     self.graph[i][j]=M_dis[nodeset[i]][nodeset[j]]
# #                     self.graph[j][i]= self.graph[i][j]
# #         if len(M_dis)==0:
# #             self.graph=graph  # HERE graph is for test here 
#         self.M_fre=M_fre
...
#             for i in range(len(nodeset)):
#                 for j in range(i+1,len(nodeset)):
#                     #self.graph[i][j]=min(self.M_fre[nodeset[i]],self.M_fre[nodeset[j]])*self.M_dis[nodeset[i]][nodeset[j]]
#                     #self.graph[i][j]=min(M_fre[nodeset[i]],M_fre[nodeset[j]])*M_dis[nodeset[i]][nodeset[j]]
...
#         result =[]
#         i,e = 0,0 
#         self.MT = sorted(self.MT,key=lambda item: item[2])
#         parent = [] ; rank = []
#         for node in range(self.V):
#             parent.append(node)
#             rank.append(0) 
#        # print(f"what is self.V {self.V}")
#         #print(f"see {parent}, {rank}")
#         while e < self.V -1 :
#             u,v,w =  self.MT[i]
#             i = i + 1
#            # print(f"which {self.MT[i]}")
...
#         self.leaf=[self.id_set[i] for i in tm]
#         
#     def addNode(self,addset):  # here dis_matrix 
#         #print(f"check {self.id_set}, {addset}")
#         #addset=[i for i in addset if i not in self.id_set]
#         #print(f"before add {self.MT}, {self.length}")
#         if len(addset)!=0:
#             seq=len(self.id_set) #from 0
#            # print(f"old V {seq}")
# #             for i in self.MT:
# #                 print(i)
#             add_matrix=[]
#             Nodes=copy.deepcopy(self.id_set)
#             for i in addset:
#                 for j in range(len(Nodes)):
#                     #fre=min(self.M_fre[i],self.M_fre[Nodes[j]])
#                    # add_matrix.append([seq, j, fre*self.M_dis[i][Nodes[j]]])
#                     add_matrix.append([seq, j, self.M_dis[i][Nodes[j]]])
#                     #print(f"I add what {addset},{seq},{ Nodes[j]},{fre*self.M_dis[i][Nodes[j]]},{fre},")
#                 seq=seq+1
#             self.id_set=self.id_set+addset
#             self.MT=self.MT+add_matrix   
#         #    print(f"wwwww to see {self.MT}")
#             # here add all possible edges, and filter that later
#            # print(f"to see",self.MT)
#             
#             self.V=self.V+len(addset)  
#            # print(f"new V {self.V}")
#             # recreate MST
#             self.KruskalMST()# renew the to_t
#             length=0
#             nodeset=copy.deepcopy(self.id_set)
#             for i in self.MT:
#                 a1=nodeset[i[0]]
#                 a2=nodeset[i[1]]
#                 fre=min(self.M_fre[a1],self.M_fre[a1])
#                 length=length+fre*i[2]
# #                 self.length=sum(i[2] for i in self.MT)
#          #   print(f"final matrix {self.MT}, {self.length} ")
#             self.length=length
#             #self.to_t=sum([self.M_fre[i]*self.To for i in self.id_set])+sum(i[2] for i in self.MT)
#             self.to_t=sum([self.M_fre[i]*self.To for i in self.id_set])+length
#             
#     def get_tsp(self):
#         nodes=[]
...
#         
# 
#     def removeParent(self,parentnodes):
#         #print("now is removing parents ooo")
#         id_order=copy.deepcopy(self.id_set)
#         parnodes=list(set(id_order)-set(self.leaf))
#         for parnode in parentnodes:
#             if parnode in parnodes:
#                 id_order.remove(parnode)
#         self.id_set=id_order
#         self.V=len(id_order)  
#         self.primMST()
#     def removeNode(self,leafnodes):    
#         #self.id_set=list(set(self.id_set)-set(leafnodes))
#        # print(f"before remove {leafnodes},{self.to_t}")
#       #  print("why here leaf", leafnodes, sorted(self.id_set) )
#         id_order=copy.deepcopy(self.id_set)
#         for leafnode in leafnodes:
#             if leafnode in self.leaf: 
# #                 print(f" here {self.id_set} , {self.leaf}")
# #                 print(f"here is leaf node {leafnode}")
#                       #, {self.id_set.index(leafnode)}")
#                 self.leaf.remove(leafnode)
#                 new_MT=[]
#                 for i in self.MT:
#                     if id_order.index(leafnode) not in [i[0],i[1]]:
#                         e1=i[0]
#                         e2=i[1]
#                         if e1>id_order.index(leafnode):
#                             e1=e1-1
#                         if e2>id_order.index(leafnode):
#                             e2=e2-1
#                         new_MT.append([e1,e2,i[2]])
# #                         self.MT.remove (i)
#                     else:
#                         self.length=self.length-i[2]
#         self.MT=new_MT
#         for leaf in leafnodes:
#             if leaf in self.id_set:
#                 self.id_set.remove(leaf)
#         #self.id_set=list(set(self.id_set)-set(leafnodes))
#         self.to_t=sum([self.M_fre[i]*self.To for i in self.id_set])+self.length
#         self.V=len(self.id_set)
#         
#         
        
        
        
       # print(f"after remove,{self.to_t}")
        
                    # self.to_t=self.to_t-  # Here I cannot control that well here
class Groups():
