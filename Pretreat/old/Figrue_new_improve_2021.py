...
#             data=[ h_min,  h_high]
        ...
        title=['Sum of Weighted AUC ', 'Sum of Weighted AUC ', #'Running time (s)',
             ...
             'Minimum sampling accuracy of high-priority targets', 'Minimum sampling accuracy', 'Minimum Detection Ratio',  'Minimum detection ratio of high-priority targets ',  ' Sum of Weighted Sampling Accuracy', ' Sum of Weighted Detection Ratio','Running Time (s)', 'Minimum Weighted AUC', 'Sum of Weighted Detection Ratio', 'Sum of Weighted Sampling Accuracy' ]
        title=['Sum of Weighted AUC ', 'Gain of Sum of Weighted AUC ', #'Running time (s)',
             'Minimum AUC of high-priority targets ',  'Average AUC ', 
             'Minimum AUC ', 'Average AUC of high-priority targets', 'Average sampling accuracy', 'Average sampling accuracy of high-priority targets', 
             'Minimum sampling accuracy of high-priority targets', 'Minimum sampling accuracy', 'Minimum Detection Ratio',  'Minimum detection ratio of high-priority targets ',  ' Sum of Weighted Sampling Accuracy', ' Sum of Weighted Detection Ratio','Running Time (s)', 'Gain of Weighted AUC (%)', 'Gain of Detection Ratio (%)', 'Gain of Sampling Accuracy (%)' ]    
...
#         Std[drone_num]=std
    ...
    legends=[]
    show=[10,2]
    legends=legends+[f"{methods[i]}"+'$_A$' for i in show]
...
#                 plt.errorbar([tt/60 for tt in T],[parp[pa][j][i]/T[j] for j in range(len(T))],yerr=[Std[k][pa][j][i]/T[j] for j in range(len(T))],fmt=f"{mark[se]}",color=col[se],capsize=2,markersize=7)
#             else:
#     #                     print(f"seee", [parp[pa][j][i] for j in range(len(T))])
...
#     legends2=[]
#     legends2=legends2+[f"{methods[i]}"+'$_A$' for i in com_a]
#     legends2=legends2+[f"{methods[i]}"+'$_K$' for i in com_a]
...
#     #plt.yticks(np.arange(0,max([parp[p][j][i]/T[j] for j in range(len(T))])+0.1 , step=0.1))
#     plt.grid( linestyle='--', linewidth=1)
#     plt.legend(legends2,loc=0,prop={'size': 15},ncol=2)
#     par='al_com'
#     filename=(f"./result/2021_result/{par}_com_size_{name[pa]}_{drone_num}_{p}_{events}_{th}.eps")
#     plt.savefig(filename,bbox_inches = 'tight')  
#     plt.show()
#     plt.clf()    
################################################################################################################   
