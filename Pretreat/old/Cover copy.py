...
from Wp_sel import M_a,Wap,get_wp,Get_D,Get_WapCa,Get_ty_WapCa,Get_TL,Calculate_inflos, GetFov, M,Get_VG_D,rot,rot_back,Figure,Write_WP, Read_D,Update_Ma
from Heuristic_drone import Get_voi, M_update, Heuristic_WpSh 
#import pickle 
...
Cov_file='/Users/fangqiliu/Desktop/Drone_code/data2/wp_cover_'
d_file='/Users/fangqiliu/Desktop/Drone_code/data/Travel_Distance2.csv'
win_fname='~/Desktop/Drone_code/data/win_ro_f_'
...
ins_m_dic={'15':{'df':0.5,'ds':0,'dh':0,'dw':0,'mf':0.3,'mh':0,'ms':0.3},'10':{'df':0.7,'ds':0.8,'dh':0.9,'dw':0.8,'mf':0.7,'mh':0.5,'ms':0.7},
           '5':{'df':1,'ds':1,'dh':1,'dw':1,'mf':1,'mh':1,'ms':1},'0':{'df':0,'dh':0,'ds':0,'dw':0,'mf':0,'mh':0,'ms':1}} 
...
# task_dic={'df1':[3,0.3],'dh1':[3,0.1],'dw1':[3,0.1],'dw2':[2,0.01],'df2':[3,0.01],'dh2':[2,0.1],
#           'mf':[5,0.5],'mh1':[4,0.5],'mh2':[4,0.3]}
task_dic={'df':[3,0.03],'ds':[3,0.0003],'dh':[3,0.0002],'dw':[3,0.0001],'mf':[5,0.05],'ms':[5,0.05],'mh':[4,0.05]}
#task_dic={'df':[3,0],'ds':[3,0],'dh':[3,0],'dw':[3,0],'mf':[5,0],'ms':[5,0],'mh':[4,0]}

State=-1*(np.ones(4)) # 1) fire 2) smoke 3) human 4) open window 
report=[]   # is locations of risky area. 

class Controller: 
    def __init__(self,all_num,lay_num,Wap_ty_set=[],report=[],task_dic={},ins_m_dic={},Dis=[],f_num=2,f_up=1,p_floor=3,w_num=32,To=To):
    ...
    def Get_WPC(self):
        ...
        if self.IM_set!=list(set(itertools.chain.from_iterable(tmp))):  # if no change, no need to change Wapc 
            ...
            for i in range(len(self.Wap_ty_set)):
                ...
                W=[];C=[]
                while len(M)>0: 
                    co.sort(key = lambda co:len(co[0]),reverse=True)  # sort first 
                    tmp=co[0][0]
        ...
        for i in list(self.Task.keys()):
            for j in list(self.Task[i]):
                if not(i in self.Ma_set[j].task): 
                    ...
                    self.Ma_set[j].va=self.Ma_set[j].va+[task_dic.get(i)[0]]
                    self.Ma_set[j].la_t=self.Ma_set[j].la_t+[-1]
    def Do_Heuristic(self,kind):
        if kind==-1:
        ...
        #ini=int(time.time())
        P=Heuristic_WpSh(Wap_set,self.Ma_set,self.Dis,self.task_dic, self.ins_m_dic,self.cu_wp,self.cu_time,self.cu_time+self.T,self.To)
        print("what P here  ",P)
        N_WP=[Wap_dic.get(i) for i in P]
        print("Generate Waypoints for next duration",self.T,"s. : ",P, "with types: ",[N_WP[i].ty for i in range(len(N_WP))])
        #print("with coverage:", [N_WP[i].cover for i in range(len(N_WP))])
        self.last_run_time=self.last_run_time+(self.T)
        TL=Get_TL([Wap_dic.get(i) for i in P],self.Dis,self.To)
        #print(TL)
        print("Travel time: ", [self.Dis[P[i-1]][P[i]]for i in range(1,len(P))],sum([self.Dis[P[i-1]][P[i]]for i in range(1,len(P))]))
        re=Calculate_inflos(self.Ma_set,TL,self.cu_time,self.cu_time+self.T,self.task_dic,self.ins_m_dic,self.To)
        print(re)
...
while ii<10:
    ...
    c=Controller(all_num,lay_num,Wap_ty,(random.sample(range(10),random.choice(range(10)))),task_dic,ins_m_dic,Dis)
    c.Get_WPC()
    c.Gen_Ma()
...
print([c.Ma_set.get(i).task for i in (list(c.Ma_set.keys()))])
# while c.cu_time<360:
