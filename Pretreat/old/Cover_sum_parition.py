...
try:
  #  T_f=int(sys.argv[1]);  # flight time 
    ...
    improve_time=int(sys.argv[3]);
    par_case=int(sys.argv[4])
  ...
  #  T_f=600
    ...
    improve_time=60
    par_case=0
Outputfile=f"result/{file_name}_{Drone_Num}_{par_case}.txt"
Drone_N=3
D=[5,15]    
#D=[5,16,24]
faces=7
...
ins_m_dic={'15':{'df':0.6,'ds':0.9,'dh':0.6,'dw':0,'mf':0.6,'mh':0.6,'ms':0.9,'mw':0},'10':{'df':0.7,'ds':0.8,'dh':0.9,'dw':0.8,'mf':0.7,'mh':0.5,'ms':0.7},
...
#             self.report=self.report+a
    ...
    def First_task(self,improve_time,par_case): # this one need to implemented! according to report, task and table. 
        self.Gen_Task()
        print(self.Task)
        self.Gen_Ma()
        self.M_partition(improve_time,par_case)
...
#plt.savefig(f"./result/partition_{self.Drone_num}_{self.T}.eps",bbox_inches = 'tight')
       # plt.show()               
    def M_partition(self,improve_time,par_case): 
        ...
        if self.Drone_num==1:
            ...
            self.partition=Partitioner(groups,M_dis,M_fre,len(Center),dic_M_W,To,improve_time,50)
            if par_case!=2:
...
#                        stop_condition=120,time_condition=True )
        ...
        return re,min_re,event_size

T=[300,600,900,1200,1500]
ii=0
improve=[0,60,180,300]
while ii<10:
    rseed=ii
...
#         Sim[row['t']]['w']=row['w']
    ...
    c.First_task(improve_time,par_case)
    for t_f in T: 
