...
for drone_num in [5]:
    for i in range(len(T)):
        ...
        for p in [0]:
            f=open(f"./result/12_result/new9_{drone_num}_{p}_{events}_{co_s}.txt",'r')
            print(f"./result/12_result/new7_{drone_num}_{p}_{events}_{co_s}.txt")
            par='new6'
            Result_min={}
            Result_ac={}
            Result_high={}
...
#              'Minimum sampling accuracy of high-priority targets', 'Minimum sampling accuracy', 'Minimum detection ratio',  'Minimum detection ratio of high-priority targets ',  ' Improved sum of weighted sampling accuracy', ' Improved sum of weighted detection ratio','Running time (s)'  ]   
        ...
        methods=['Max_ac','Max_cov+Max_min','MI','NN','Max_cov+Greedy_min','MM','Greedy_grain','Greedy_weight_grain','DWS','Max_cov+Greedy_weight_grain','DWSF','MST','MST','$WM^{C+}$']
        ...
        for pa in [2,4,1,-3,-2,-1]: #range(len(parp)):#  #  
        #for pa in [-5]: 
            for n in show:
            ...
            filename=(f"./result/2021_result/{par}_{name[pa]}_{drone_num}_{p}_{events}_{co_s}.eps")
...
for pa in [1,-3,-2,-1]: #range(len(parp)):#  #  
    ...
    filename=(f"./result/24_result/{par}_com_size_{name[pa]}_{drone_num}_{p}_{events}_{co_s}.eps")
    #plt.savefig(filename,bbox_inches = 'tight')  
    plt.show()
    plt.clf()    
################################################################################################################   
