...
class Controller: 
    ...
    def Coordiniation(self):
        ...
        for i in range(self.Drone_num):
            ...
            drone=Drone(self.Wap_ty_set,M_set,self.Dis,self.task_dic,self.ins_m_dic,self.cu_wp,self.cu_time,self.T)
            drone.Get_WPC()
    def Get_WPC(self):
        #Wap_ty=[]; ;Cov_ty=[];Cov=[]
...
class Drone:     # WPC   # All Monitoring areas with tasks
    ...
    def Get_WPC(self):        #print(self.Task,self.Table)
        self.IM_set=[i.id for i in self.Ma_set.values()]
        print(f" to see {self.id},{self.IM_set}")
