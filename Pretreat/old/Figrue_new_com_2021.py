...
#for events in [1,2,3]:
#for drone_num in [3,5,7]
#for th in threshold:
for p in [0,2]:#threshold:
    for i in [0]:#range(len(T)):
        ...
        for th in [1]:#threshold:
            if p==2:
                ...
                f=open(f"./result/2021_new_result/new1_{drone_num}_{p}_{events}_{th}.txt",'r')
                print(f"./result/12_result/new2_{drone_num}_{0}_{events}_{th}.txt")
            elif drone_num==5 and p==1:
                f=open(f"./result/12_result/21new3_{drone_num}_{1}_{events}_{th}.txt",'r')
            else:
    ...
    #                 output.write(f"{drone.id} {i} {j} {re} {min_re[0]} {min_re[1]} {high_sum} {high_size} {miss} {min_high} {sum_w_v} {run_time} {s_av} {s_h_av} {s_m_h} {s_m_ac} {s_lm_d} {s_hm_d}\n")
                    ...
                    run_time=float(a[12])
                    if fre==1500 and method==10:
                        print(f"seee",run_time)
                    sam_av=float(a[13])
                    sam_h_av=float(a[14])
...
#              'Minimum sampling accuracy of high-priority targets', 'Minimum sampling accuracy', 'Minimum detection ratio',  'Minimum detection ratio of high-priority targets ',  ' Improved sum of weighted sampling accuracy', ' Improved sum of weighted detection ratio','Running time (s)'  ]   
        ...
        if p==0:
            ...
            legends=['KM+'+methods[i] for i in show]
        hfont = {'fontname':'Numbers','size':'15'}
#         #for pa in [3,4,-3,1,-6,-5,-4,-2,-1]: #range(len(parp)):#  #  
#         for pa in [1,-4,-3,-2,-1]:
#               for n in show:
    ...
    for k, parp in Par.items():
        for n in com_a:
            ...
            if pa in [0,1,2,3,4,5,-3]:     
                ...
                if k==0:
                    print([parp[pa][j][i]/T[j] for j in range(len(T))])
                    b=[Par[2][pa][j][i]/T[j] for j in range(len(T))]
                    a=[parp[pa][j][i]/T[j]-Par[2][pa][j][i]/T[j] for j in range(len(T))]
                    c=[a[i]/b[i]  for i in range(1,len(s))]
                    print(max(c)*100)
                    print([(parp[pa][j][i]/T[j]-(Par[2][pa][j][i]/T[j]))/(Par[2][pa][j][i]/T[j]) for j in range(len(T))])

            else:
    #                     print(f"seee", [parp[pa][j][i] for j in range(len(T))]) color=col[se]
                ...
                if k==0:
                    print([parp[pa][j][i] for j in range(len(T))])
                    b=[Par[2][pa][j][i] for j in range(len(T))]
                    a=[parp[pa][j][i]-Par[2][pa][j][i] for j in range(len(T))]
                    c=[a[i]/b[i]  for i in range(1,len(s))]
                    print(max(c)*100)
                    print([(parp[pa][j][i]/T[j]-(Par[2][pa][j][i]))/(Par[2][pa][j][i]) for j in range(len(T))])
...
#     legends3=[methods[i]+'$_{W}$'+f" {k}s Time" for k in [1,30,60,120]]
...
#           )
    ...
    legends2=[]  
    legends2=legends2+[f"AMT+{methods[i]}" for i in com_a]
    legends2=legends2+[f"KM+{methods[i]}" for i in com_a]
    hfont = {'fontname':'Numbers','size':'15'}
    plt.xlabel('Plan Duration (Minute)',**hfont)
    plt.ylabel(title[pa],**hfont)
    #plt.yticks(np.arange(0,max([parp[p][j][i]/T[j] for j in range(len(T))])+0.1 , step=0.1))
    plt.grid( linestyle='--', linewidth=1)
    plt.legend(legends2,prop={'size': 15},ncol=2)
    #par='al_com'
    filename=(f"./result/2021_result/{par}_{name[pa]}_{drone_num}_{p}_{events}_{th}.eps")
    plt.savefig(filename,bbox_inches = 'tight')  
    plt.show()
