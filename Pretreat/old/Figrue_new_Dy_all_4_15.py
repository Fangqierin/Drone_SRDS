...
#             plt.clf() 
            ...
            for me in total_m:#[w_AUC,w_sample,w_dect,w_min_AUC]:
                ...
                plt.ylabel(metrics[cc])
                plt.xlabel('Methods')
                filename=(f"./result/2022_result/{par}_names[cc]_{drone_num}.eps")
                plt.savefig(filename,bbox_inches = 'tight')  
                plt.show()
                plt.clf() 
                cc=cc+1  
...
#     legends3=[methods[i]+'$_{W}$'+f" {k} tasks" for k in [339,521,632]]
        ...
        mark[2]=['--o','--*']
          #  col=['tab:blue','tab:green']
                ...
                #print(f"Par {k} {miss}")
#             for kk in range(len(Par2[k])):
#                 se=0
                #print(f"see dat",kk,len(Par2[k][kk]))
    ####################################################### Compare with time, 
...
#                     print([Par[k][pa][j][i] for j in range(len(T))])
#                     print([Par_error[k][pa][j][i] for j in range(len(T))])
                    plt.plot([tt/60 for tt in T],[Par2[k][pa][j][i] for j in range(len(T))],mark[k][se],markersize=7)
