...
for drone_num in [2,4]:
    for va in range(1,5):
        f=open(f"./result/num_{drone_num}_{va}.txt",'r')
        par='num'
        #     k=[0,1,3,4]
            #print(k.index(0))
        ...
        for k in range(10):
            ...
            Result_size[k]=copy.deepcopy(fre_dic)
        for line in f: 
    ...
    #             run_time=float(a[7])
                ...
                Result_size[num][fre][method].append(event_size)
        ...
        c_size=copy.deepcopy(Result_size)
        for i in Result_min:
        ...
        for k,s in c_size.items():
            for t in range(len(T)):
                h_size[t].append(list(s[T[t]].values()))
        #print(c_high)
        #print(f"h_high",h_high)
        av_ha=[];min_ha=[];high_ha=[];min_high_ha=[];w_min_ha=[];w_sum_ha=[];e_size=[]
        #print(f"check h_size",h_size)
        for i in h_size:
            tmp=[]
    ...
    #Num_result.append([av_ha[j][5]/T[j] for j in range(len(T))])
        methods=['Max_ac','Max_cov+Max_min','Greedy_min','Greedy_TSP','Max_cov+Greedy_min','Max_min','Greedy_grain','Greedy_weight_grain','Max_cov+Greedy_min']

        #methods=['Max_ac','Max_cov+Max_min','Greedy_min','Greedy_TSP','Max_cov+Greedy_min','Max_min','Greedy_grain','Greedy_weight_grain','Greedy_weight_min','Max_Cov+Greedy_weight_grain','Max_Cov+Greedy_min']
        mark=['+','*','o','.','^','o','+','*','o','.','^','o']
        ...
        for i in show:
           ...
           # print([av_ha[j][i] for j in range(len(T))])
            plt.plot(T,[e_size[j][i] for j in range(len(T))],marker=mark[i])
        ...
        filename=(f"./result/new_result/{par}_w_sum_{drone_num}_{va}.eps")
...
#         
        ...
        plt.grid( linestyle='--', linewidth=1)
        plt.legend(legends,loc=0,prop={'size': 8},ncol=2)
        filename=(f"./result/new_result/{par}_w_min_{drone_num}_{va}.eps")
        plt.savefig(filename,bbox_inches = 'tight')  
        plt.show()
        plt.clf() 
        ...
        plt.grid( linestyle='--', linewidth=1)
        plt.legend(legends,loc=0,prop={'size': 8},ncol=2)
        filename=(f"./result/new_result/{par}_min_high_{drone_num}_{va}.eps")
        plt.savefig(filename,bbox_inches = 'tight')  
        plt.show()
        plt.clf() 
        ...
        plt.grid( linestyle='--', linewidth=1)
        plt.legend(legends,loc=0,prop={'size': 8},ncol=2)
        filename=(f"./result/new_result/{par}_Av_sum_{drone_num}_{va}.eps")
        plt.savefig(filename,bbox_inches = 'tight')  
        plt.show()
        plt.clf()  
        for i in show:
        ...
        plt.grid( linestyle='--', linewidth=1)
        plt.legend(legends,loc=0,prop={'size': 8},ncol=2)
        filename=(f"./result/new_result/{par}_Min_ac2_{drone_num}_{va}.eps")
        plt.savefig(filename,bbox_inches = 'tight')  
        plt.show()
        plt.clf()  
        ...
        plt.grid( linestyle='--', linewidth=1)
        plt.legend(legends,loc=0,prop={'size': 8},ncol=2)
        filename=(f"./result/new_result/{par}_High_ac_{drone_num}_{va}.eps")
        plt.savefig(filename,bbox_inches = 'tight')  
        plt.show()
        plt.clf() 
