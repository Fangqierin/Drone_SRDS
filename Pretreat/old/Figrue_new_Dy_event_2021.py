...
# for i in range(len(T)):
    ...
    for size in [1]:
        for p in [2]:
            if p==2:
                f=open(f"./result/result_2021/dy4_177_{drone_num}_{size}_{policy}_{p}.txt",'r')
            else:
...
#                         print(f"sim_fire",a)
    ...
    ###############################################################################  
            ...
            show=[11,9,6,4,12]
            if p==0:
                legends=['AMT+'+methods[i-1] for i in show]
            elif p==2:
                legends=['KM+'+methods[i-1] for i in show]
            mark=['-o','-*','-->','--p','-.^','--+','--<','-.*','--^','-.','-^','-^','-*','->','-*'] 
            #mark=['o','*','>','p','^','<','+','*','--+','-.','-^','-^','-*','->','-*'] 
# #################################################################### Missing event. 
            f = plt.figure()
...
#                     plt.errorbar([tt/60 for tt in T],[parp[pa][j][i] for j in range(len(T))],yerr=[std[pa][j][i] for j in range(len(T))],fmt=f"{mark[se]}",capsize=2,markersize=7)
            ...
            plt.xlabel('Time (Minute)',**hfont)
            plt.ylabel('Missing Events',**hfont)
            plt.axis('tight')
            ...
            plt.legend(legends,loc=1,prop={'size': 14},ncol=2)
            par='Dy_slot'
            ...
            plt.legend(legend,prop={'size': 10.5},ncol=4)
            par='Dy_event'
            filename=(f"./result/2021_dy/{par}_{drone_num}_{size}_{policy}_{p}.eps")
            #plt.savefig(filename,bbox_inches = 'tight') 
            #plt.show()
...
#######################################################################         
        ...
        #plt.yscale('symlog')
        if size==2:
            legends2=legends2+['AMT+'+methods[i-1] for i in show1]
            legends2=legends2+['KM+'+methods[i-1] for i in show1]
        if size==3:
            legends2=legends2+['AMT+'+methods[i-1] for i in show1]
            legends2=legends2+['KM+'+methods[i-1] for i in show1]
...
#     legends3=[methods[i]+'$_{W}$'+f" {k} tasks" for k in [339,521,632]]
    ...
    plt.ylabel('Missing Events',**hfont)
    plt.grid( linestyle='--', linewidth=1)
    plt.legend(legends2,loc=0,prop={'size': 14},ncol=2)
    par='Dy_miss_col'
...
#     legends3=[methods[i]+'$_{W}$'+f" {k} tasks" for k in [339,521,632]]
    #plt.ylim(0)       
    plt.xlabel('Time (Minute)',**hfont)
    plt.ylabel('Accumulative Missing Events',**hfont)
    plt.grid( linestyle='--', linewidth=1)
    plt.legend(legends2,prop={'size': 14},ncol=2)
    par='Dy_col_4'
    filename=(f"./result/2021_dy/{par}_com_al_{drone_num}_{size}.eps")
    plt.savefig(filename,bbox_inches = 'tight')  
    plt.show()
