...
def Figure(building, W_set):
    ...
    graph=vg.VisGraph()
    graph.build([polygen])
    plt.plot([polygen[i].x for i in list(range(len(polygen)))+[0]],[polygen[i].y for i in list(range(len(polygen)))+[0]])
    plt.scatter([W_set[i][0]for i in range(len(W_set))],[W_set[i][1] for i in range(len(W_set))])
    ...
    plt3d.plot_surface(xx, yy, zz, alpha=0.2)
    plt3d.scatter([W_set[i][0]for i in range(len(W_set))],[W_set[i][1] for i in range(len(W_set))],
        [W_set[i][2]for i in range(len(W_set))])
...
def Read_D(d_file, R_v):
    Dis=pd.read_csv('~/Drone.git/trunk/data/Travel_Distance.csv',sep=' ')
    i=list(Dis['i'])
    j=list(Dis['j'])
    d=list(Dis['d'])
...
####### old 
def Calculate_inflos_old(M_set,Tl,Ti,Time,task_dic,ins_m_dic,To):# here the every time, the inital value is 0. We already give inital value
    TL=copy.deepcopy(Tl)   # T is the coverge time list, there is no waypoints issue. 
    ma_set=copy.deepcopy(M_set)
    Sum=0
