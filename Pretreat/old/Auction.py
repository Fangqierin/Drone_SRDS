        ...
        for i in [agent1,agent2]:
            ...
            if len(id)==0:
                ...
                if self.method==0:
                    ...
                    Group.Center[i]=None
        return Group
    def addNode(self,Group,node,agent):
        #Group=copy.deepcopy(Groups)
        M_set_split=copy.deepcopy(Group.M_set_split)
        i=agent
...
#             Group.Center[i]=id[ind]
    ...
    def do_improve(self):
        ...
        def stop_condition():
            ...
            return num_iter>self.Max_iter or time.time()-t_start>self.Max_time or no_improve>=3
    ...
    #         while not stop:
           ...
           # print(f"where is leaf {leafs}, {agents_re}")
            ...
            if va<=0.95:
                ...
                for i in area2:
                    to_sort=[ii for ii in area2 if ii!=i]
                    #andom.shuffle(to_sort)
    ...
    #                 for j in agents_re:
                            ...
                            #print(f"Best_p channged? {max(Best_p.to_t)}")
                            max_tt=max(new_group.to_t)
...
#                                 print(f" see transfer leaf {a} from {i} to {j} is {max_tt} {(max_t-max_tt)-(sum_tt-sum_t)}")
                            #print(f"whyyy {max_tt,max_t}")
                            if max_tt<max_t and  sum_tt<=sum_t*1.003 :#and (max_t-max_tt)-(sum_tt-sum_t)>net:
...
#                 print(Best_p.M_set_split,Best_p.Center)
        self.Groups=Best_p
        return Best_p,max_t,sum_t,np.std(list(Best_p.to_t)),list(Best_p.to_t)
