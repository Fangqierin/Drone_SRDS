...
#             self.report=self.report+a
    ...
    def First_task(self): # this one need to implemented! according to report, task and table. 
        ...
        self.Gen_Ma()
        self.M_partition()
    def Coordiniation(self,T):
        Drones=[]
        for i in range(self.Drone_num):
            M_set=dict(zip(self.M_set_split[i],[self.Ma_set.get(i) for i in self.M_set_split[i]]))
            print(f" to check M_set {M_set}")
            drone=Drone(i,self.Wap_ty_set,M_set,self.Dis,self.task_dic,self.ins_m_dic,self.cu_wp,self.cu_time,T)
            Drones.append(drone)
           # drone.Get_WPC()
        return Drones
    def Gen_Ma(self):
...
class Drone:     # WPC   # All Monitoring areas with tasks
    def __init__(self,id,Wap_ty_set,Ma_set,Dis,task_dic,ins_m_dic,cu_wp,cu_time,T):
        self.id=id
        self.Wap_ty_set=Wap_ty_set
        self.Ma_set=Ma_set
        self.Dis=Dis
