...
#              'Minimum sampling accuracy of high-priority targets', 'Minimum sampling accuracy', 'Minimum detection ratio',  'Minimum detection ratio of high-priority targets ',  ' Improved sum of weighted sampling accuracy', ' Improved sum of weighted detection ratio','Running time (s)'  ]   
        ...
        #for pa in [4,1,-3,-2,-1]: #range(len(parp)):#  #  
        for pa in [-1]: 
            for n in show:
       ...
       # for pa in [4]: 
        for n in [10,2]:
            ...
            if pa in [0,1,2,3,5]:     
                ...
                plt.errorbar([tt/60 for tt in T],[parp[pa][j][i]/T[j] for j in range(len(T))],yerr=[Std[k][pa][j][i]/T[j] for j in range(len(T))],fmt=f"{mark[se]}",capsize=2,markersize=7,color=col[se])
            else:
    #                     print(f"seee", [parp[pa][j][i] for j in range(len(T))])
                plt.errorbar([tt/60 for tt in T],[parp[pa][j][i] for j in range(len(T))],yerr=[Std[k][pa][j][i] for j in range(len(T))],fmt=f"{mark[se]}",capsize=2,markersize=7,color=col[se])
            se=se+1
