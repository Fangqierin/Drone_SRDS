...
from Heuristic_drone_weighted import Greedy_WPS,Tabu,Get_TL,Greedy_Min,Tabu_min,Calculate_Acc, Greedy_TSP,Sample_Acc
...
Outputfile=f"result/12_result/{file_name}_{Drone_Num}_{par_case}_{va}_{c_t}.txt"
Drone_N=3
D=[5,15]    
#D=[5,16,24]
faces=7
...
if c_t==3:
    task_dic={'df':[1,0.005],'ds':[1,0.005],'dh':[1,0.005],'dw':[1,0.005],'mf':[3,0.015],'mh':[3,0.015],'dw2':[3,0.008],'df2':[3,0.01],'dh2':[3,0.01],'mw':[3,0.005]}
if c_t==2:
...
co_s=1
class Controller: 
    def __init__(self,all_num,lay_num,Wap_ty_set=[],report=[],hum=[],task_dic={},ins_m_dic={},Dis=[],f_num=1,f_up=1,p_floor=1,w_num=32,To=To,T=0,Drone_num=0,Dic_M_W=dic_M_W):
...
#             self.report=self.report+a
        ...
        for i in list(tmp):
            ...
            print(f"see where is human {hum_set}")
...
#         self.Task['mh'].update(list(self.Table['human']))
    ...
    def draw (self,M_set_split,Center=[],num=[],mat_f=None):
        output=open(mat_f,'w+')
        co=['b','g','orange','black','y','b','g','orange','black','y']
...
#             ax.scatter([dic_M_W.get(i)[0] for i in Center],[dic_M_W.get(i)[1] for i in Center],[dic_M_W.get(i)[2] for i in Center],color='black',s=50)
        if num!=[]:
            seq=0; color=['red','red'];mak=['^','*']
            for i in range(len(num)):
                output.write(str(len(num[i]))+' ')
            output.write('\n') 
            print(f" see fire or human {num}")
            for o in num:
                for j in o:
                    output.write(f"{dic_M_W.get(j)[0]} {dic_M_W.get(j)[1]} {dic_M_W.get(j)[2]}\n")
                ax.scatter([dic_M_W.get(i)[0] for i in o],[dic_M_W.get(i)[1] for i in o],[dic_M_W.get(i)[2] for i in o],color=color[seq],marker=mak[seq],s=50)
                seq=seq+1
        output.close()
#plt.savefig(f"./result/partition_{self.Drone_num}_{self.T}.eps",bbox_inches = 'tight')
    ...
    def M_partition(self,par_case): 
        ...
        M_dis=self.M_dis
        M_set_split=[[] for i in range(min(self.Drone_num,len(self.Ma_set)))]
        ...
        if self.Drone_num==1:
            ...
            M_fre={}  
            print(f"size of {len(self.Ma_set)}")
            ...
            groups.Get_initial()
            self.draw(groups.M_set_split, groups.Center,[list(self.Task['mf']),list(self.Task['mh'])],f"../drone_matlab/first_output_{self.Drone_num}_{self.T}.txt")
            partition=Partitioner(groups,M_dis,M_fre,len(Center),dic_M_W,To,1800,40)
            #print(f"groups ",groups.M_set_split) 
            if par_case!=2:
                groups=partition.do_improve()
            self.draw(groups.M_set_split, groups.Center,[list(self.Task['mf']),list(self.Task['mh'])],f"../drone_matlab/partition_output_{self.Drone_num}_36.txt")
...
while ii<20:   ####################### for test
    ...
    pa_t=c.First_task(par_case)
    for t_f in T: 
        Drones,Wap_dic=c.Coordiniation(t_f,1500)
        output.write(f"12random:{rseed}T:{t_f} {pa_t}\n")
        for i in range(-1,0):
            if i ==-1:
                for j in range(14):
                    TL_list=[]; P_list=[]
...
#                         print(i,j)
                    run_time=max(run_t)
                    print(f"to see" ,t_f, i,j,run_t)
                    #re,min_re,event_size,high_sum,high_size,min_high,sum_w_v,w_min_v,min_dict=Calculate_Acc(self.Ma_set,TL,self.cu_time,self.cu_time+self.T,self.task_dic,self.ins_m_dic,high_task)
                    re,min_re,miss,high_sum,high_size,min_high,sum_w_v,w_min_v,w_task_min=Calculate_Acc(c.Ma_set,Tl_com,c.cu_time,c.cu_time+t_f,c.task_dic,c.ins_m_dic,high_task)
                    s_av,s_h_av,s_m_h,s_m_ac,s_lm_d,s_hm_d,min_l_d,min_h_d,w_sample_min,w_detect_min,High_va,Va_set=Sample_Acc(c.Ma_set,Tl_com,c.cu_time,c.cu_time+t_f,c.task_dic,c.ins_m_dic,high_task,sample_rate,detect_threshold)
                    j=j+1
                    output.write(f"{drone.id} {ii} {j} {re} {min_re[0]} {min_re[1]} {high_sum} {high_size} {miss} {min_high} {sum_w_v} {w_min_v} {run_time} {s_av} {s_h_av} {s_m_h} {s_m_ac} {s_lm_d} {s_hm_d} {w_task_min} {min_l_d} {min_h_d} {w_sample_min} {w_detect_min} \n")
                    ...
                    print(f"fangqi to see ", w_min_v)
                    output.write(f"high {High_va}\n")
                    output.write(f"low {Va_set}\n")
                    
#                     print(f"see combined TL", (Tl_com))
                    #print(f" fangqiiii see", t_f, j, re,min_re,miss,high_sum,high_size,min_high,sum_w_v,w_min_v)
