...
# for i in range(len(T)):
    f=open(f"./result/par4_{drone_num}_0.txt",'r')
    par='par4'
    #     k=[0,1,3,4]
        #print(k.index(0))
...
#     for i in h_av:
#         #print(f"see what is that {i}")
#         tmp=[]
#         for k in range(len(i[0])):
#             print(len(i[0]))
#             #print(f"see what is that", [i[f][k] for f in range(len(i))])
#             tm=round(np.mean([i[f][k] for f in range(len(i))]),2)
#             sd=np.std([i[f][k] for f in range(len(i))])
#             #print(f"so {tm} {conf_co*sd/math.sqrt(len(i[0]))}\n")
#             tm=round(np.mean([i[f][k] for f in range(len(i))]),2)
#             sd=np.std([i[f][k] for f in range(len(i))])
#             #print(f"so {tm} {conf_co*sd/math.sqrt(len(i[0]))}\n")
#             tmp.append(tm)
...
#     parp=[ w_sum_ha, w_min_ha,  min_high_ha, av_ha, min_ha, high_ha]
#     parp=[ min_ha]
    print(f" see {parp}")
    title=[' Sum of weighted accumulative accuracies ',' Sum of weighted minimum AUC ',
         'Minimum accumulative accuracy of high-priority tasks ',  'Average accumulative accuracy ', 
         'Minimum accumulative accuracy ', 'Average accumulative accuracy of high-priority tasks ']   
    name=['w_sum','w_min', 'min_high', 'Av_sum',  'Min_ac','High_ac']
    methods=['Max_ac','Max_cov+Max_min','Greedy_min','Greedy_TSP','Max_cov+Greedy_min','Max_min','Max_cov+Greedy_grain','Greedy_weight_grain','Greedy_min','Max_Cov+Greedy_weight_grain','Max_Cov+Greedy_min']
    mark=['-+','-*','-o','-o','-^','-o','-+','-*','-o','-.','-^','-o']
    show=[7,3,4,6,5]
    show=[7,8,3,4,6,5]
    show=[7,8,9,10]
    legends=[methods[i] for i in show]
    for p in [1]:#range(len(parp)): 
        for i in show:
        ...
        #print([av_ha[j][i] for j in range(len(T))])
            ...
            plt.errorbar(T,[parp[p][j][i]/T[j] for j in range(len(T))],yerr=[std[p][j][i]/T[j] for j in range(len(T))],fmt=f"{mark[i]}",capsize=2)
        ...
        filename=(f"./result/10_result/{par}_{name[p]}_{drone_num}.eps")
    ...
    plt.grid( linestyle='--', linewidth=1)
    plt.legend(legends,loc=0,prop={'size': 8},ncol=2)
    filename=(f"./result/new_result/{par}_Min_ac2_{drone_num}.eps")
    plt.savefig(filename,bbox_inches = 'tight')  
    plt.show()
    plt.clf()  
