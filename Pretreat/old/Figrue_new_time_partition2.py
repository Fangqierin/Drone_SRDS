...
# for i in range(len(T)):
    ...
    for tim in time:
        print(f"./result/12_result/new7_{drone_num}_{p}_{events}_{tim}.txt")
        f=open(f"./result/12_result/new8_{drone_num}_{p}_{events}_{tim}.txt",'r')
...
#                     plt.errorbar(T,[parp[pa][j][i] for j in range(len(T))],yerr=[std[pa][j][i] for j in range(len(T))],fmt=f"{mark[i]}",capsize=2,markersize=7)
...
#for pa in range(len(parp)):#  #  
    ...
    #                     print(f"seee", [parp[pa][j][i] for j in range(len(T))])
                plt.errorbar(T,[parp[pa][j][i] for j in range(len(T))],yerr=[Std[k][pa][j][i] for j in range(len(T))],fmt=f"{mark[se]}",capsize=2,markersize=7)
            se=se+1
    legends3=[methods[i]+'$_{W}$'+f" {k} time" for k in [1,30,60,120,180]]
