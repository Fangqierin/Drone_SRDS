...
class Controller: 
    def __init__(self,all_num,lay_num,Wap_ty_set=[],report=[],hum=[],task_dic={},ins_m_dic={},Dis=[],f_num=2,f_up=1,p_floor=3,w_num=32,To=To,T=0,Drone_num=0,Dic_M_W=dic_M_W):
...
#plt.savefig(f"./result/partition_{self.Drone_num}_{self.T}.eps",bbox_inches = 'tight')
    ...
    def M_partition(self): 
        ...
        if self.Drone_num==1:
            ...
            self.partition=Partitioner(groups,M_dis,M_fre,len(Center),dic_M_W,To,180,50)
...
#         print(tmp)
    ...
    def Get_Interval(self,Tl):
        ...
        for i in ma_set:
            ...
            sort_fre[tmp].append(i)
        #print(sort_fre)
        for i,s in sort_fre.items():
            for j in s:
                if Tl.get(j)!=None:
                    timeline=[t[0] for t in Tl.get(j)]
                    #print([k for k in Tl.get(j) if self.ins_m_dic.get(str(k[1])).get(i[:2])!=0])
                    times=len([k for k in Tl.get(j) if self.ins_m_dic.get(str(k[1])).get(i[:2])!=0])
                    t=ma_set.get(j).start; interval=[]
                    for k in range(len(timeline)):
                        interval.append(timeline[k]-t)
                        t=timeline[k]
                    Track_visit[i][j]=times
                    Track_interval[i][j]=interval
        for i, j in Track_interval.items():
            print(i,Track_interval.get(i))   
...
#                        stop_condition=120,time_condition=True )
        ...
        re,min_re,event_size,high_sum,high_size=Calculate_Acc(self.Ma_set,TL,self.cu_time,self.cu_time+self.T,self.task_dic,self.ins_m_dic,high_task)
        print(f"{kind},{case},End with reliability value",re,"min_re:",min_re,"at",cu_t)
        return re,min_re,event_size,TL,high_sum,high_size
...
#     c.Add_Task(tasks)
    ...
    for t_f in T: 
        ...
        for drone in Drones:
            ...
            for i in range(-1,0):
                if i ==-1:
                    for j in range(6): #range(4):
                        t=time.time()
                        re,min_re,miss,TL,high_sum,high_size=drone.Do_Heuristic(i,j) # do no update task list. 
                        tt=time.time()-t
                        print("finish first one with",i,drone.id,time.time()-t)
                        TL_list.append(TL)
                        output.write(f"{drone.id} {i} {j} {re} {min_re[0]} {min_re[1]} {miss} {high_sum} {high_size} {tt} {ii}\n")
                    #c.Get_Interval(TL_list)
