...
#         print("Wap",Wap_set[i].id)
    ...
    return P,cu_t
#################### 5/24 for max min 
def Greedy_TSP(case,Wap_set,M_set,D,task_dic,ins_m_dic,w_in,T_in,T_tm):
    pass
    ...
    for i in cover:
        ...
        for j in range(len(task_set)):
            ...
            if T_rest==0:
                ...
                Grain=Grain+(new_re-pre_re)
                m_new_set[i].ac_re[j]=mac_re[j]+(new_re-pre_re)
        ...
        if TL.get(k)==None:   # if areas did not be covered 
            for j in range(len(task_set)):  # for all tasks of a m 
                ...
                if b>0:
                    ...
                    dur=Time-Ti # here oooo
                    ma_set[k].va[j]=max(0,b-dur*dy)  # oooo dur is not right! 
        else:  
            for j in range(len(task_set)):  # for all tasks of a m 
                sum_one=0
                [si,dy]=task_dic.get(task_set[j])    
    ...
    for i in wap.cover:
        ...
        for j in range(len(task_set)):
            ...
            if T_rest==0:
                ...
                d_t=(T_rest if dy==0 else min(T_rest,new/dy))
                new_re=(new+max(0,new-T_rest*dy))*d_t*0.5
                m_co[i].ac_re[j]=mac_re[j]+(new_re-pre_re)
            #print(i,mac_re[j],m_co[i].ac_re[j],acc,tmp,dy)          
    return m_co,(arr_t) # here is leave time
...
def Tabu_min(P_in,Wap_set,M_set,D,task_dic,ins_m_dic,w_in,T_in,T_tm,NeigSize,MaxTabuSize,stopTurn,
    ...
    while not stop_condition():
        ...
        bestCandidate = sNeighborhood[0]
        for sCandidate in sNeighborhood:
            if (sCandidate not in tabuList) and (fitness_min(sCandidate, Wap_set,M_set,D,task_dic,ins_m_dic,w_in,T_in,T_tm)) > bestfitness:
                bestCandidate = sCandidate
...
def fitness(P,Wap_set,M_set,D,task_dic,ins_m_dic,w_in,T_in,T_tm):
    ...
    re,min_re,miss=Calculate_Re(M_set,Tl,T_in,T_tm,task_dic,ins_m_dic)
    return re
...
def Update_M_TL_new(M_set,Tl,Ti,Time,D,task_dic,ins_m_dic,T_tm):
...
def Get_Re_Min_new(case,wap,m_set,cu_t,D,w_id,task_dic,ins_m_dic,T_tm):
...
def M_update_Min_new(wap,ma_set,cu_t,D,w_id,task_dic,ins_m_dic,T_in,T_tm):
    m_co=copy.deepcopy(ma_set)
    arr_t=cu_t+D[w_id][wap.id]
    #print(f"{wap.cover} at{wap.id} at time{cu_t}")
    T_rest=T_tm-(cu_t+D[w_id][wap.id])
