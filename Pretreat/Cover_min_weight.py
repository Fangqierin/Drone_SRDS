...
try:
  #  T_f=int(sys.argv[1]);  # flight time 
   ...
   # rseed=int(sys.argv[2]) # random seed 
    ...
    file_name=''
    co_s=0.34
Outputfile=f"result/{file_name}_{Drone_Num}_{co_s}.txt"
Drone_N=3
D=[5,15]    
#D=[5,16,24]
faces=7
...
#plt.savefig(f"./result/partition_{self.Drone_num}_{self.T}.eps",bbox_inches = 'tight')
    ...
    def M_partition(self): 
        ...
        if self.Drone_num==1:
            ...
            partition=Partitioner(groups,M_dis,M_fre,len(Center),dic_M_W,To,180,50)
...
class Drone:     # WPC   # All Monitoring areas with tasks
    ...
    def Do_Heuristic(self,kind,case,co_s):            
        ...
        if case==3:
            ...
            P,cu_t=Greedy_Min(case,co_s,Wap_set,self.Ma_set,self.Dis,self.task_dic, self.ins_m_dic,self.cu_wp,self.cu_time,self.cu_time+self.T,self.T_total)
#         if case==5: 
...
#                        stop_condition=120,time_condition=True )
        ...
        re,min_re,event_size,high_sum,high_size,min_high,sum_w_v,w_min_v,min_set=Calculate_Acc(self.Ma_set,TL,self.cu_time,self.cu_time+self.T,self.task_dic,self.ins_m_dic,high_task)
        print(f"{kind},{case},End with reliability value",re,"min_re:",min_re,"at",cu_t)
        return re,min_re,event_size,TL,high_sum,high_size,min_high,sum_w_v,w_min_v,min_set        
...
while ii<10:
    ...
    for t_f in T: 
        ...
        for i in range(-1,0):
            if i ==-1:
                for j in range(9):#range(4):
                    TL_list=[]
                    min_dict=defaultdict(list)
                    for drone in Drones:
                        drone.Get_WPC()
                        t=time.time()
                        re,min_re,miss,TL,high_sum,high_size,min_high,sum_w_v,w_min_v,min_set=drone.Do_Heuristic(i,j,co_s) # do no update task list. 
                        for q in min_set:
                        ...
                        #print(f"seee {drone.id} {i} {j} {min_re[0]} {min_high} ")
                        #TL_list.append(TL)
                        #TL_list.append(TL)
