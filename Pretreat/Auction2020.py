...
import time      
class Groups():
    def __init__(self,id_set,Centers,M_dis,M_fre,dic_M_W,To,method):
        self.M_dis=M_dis
        self.M_fre=M_fre
        self.Center=Centers
        self.id_set=id_set
        ...
        self.method=method
    def draw (self,M_set_split,Center,filename):
    ...
    def Get_initial(self):
        ...
        if self.method!=0:
            ...
            for i in range(len(self.M_set_split)):
                ...
                D_sum=0
                for a in range(len(id)):
                    for b in range(a+1,len(id)):
                            D_sum=D_sum+max(self.M_fre[id[b]],self.M_fre[id[a]])*self.M_dis[id[a]][id[b]]
                D_sum=(D_sum*2)/(len(id)-1)
                new_to_t[i]=D_sum
            self.to_t=new_to_t
...
#          
class Partitioner():
    def __init__(self,Groups,M_dis,M_fre,Drone_num,dic_M_W,To,Max_time,Max_iter,method):
        self.Groups=Groups
        self.M_dis=M_dis
        self.M_fre=M_fre
 #       self.id_set=id_set
...
#         Group.Center[i]=id[ind]
        id=M_set_split[i]
        #print(f"what contains in that {id}")
        if len(id)==0:
            Group.to_t[i]=0
            Group.Center[i]=None
        else:
            if self.method==0:
                ...
                for a in range(len(id)):
                    for b in range(len(id)):
                        if a!=b:
                            #print(id[a],id[b])
                            #print(f" why {a} {b} {self.M_fre}, {self.M_fre[id[b]]} {id[a]} {id[b]}, {self.M_dis[id[a]][id[b]]}")
                            D[a][b]=self.M_fre[id[b]]*self.M_dis[id[a]][id[b]]
                    dis=sorted([(sum(D[i][:]),i) for i in range(len(D))])
                    Group.to_t[i]=dis[0][0]+sum([self.M_fre[id[k]]*self.To for k in range(len(id))])
                    ind=dis[0][1]
...
#             Group.Center[i]=id[ind]
    ...
    def draw (self,M_set_split,Center,filename):
        co=['b','g','orange','black','y','b','g','orange','black','y','b','g','orange','black','y']
        fig=plt.figure()
        ax=fig.add_subplot(111,projection='3d')
    #             ax.scatter([M_loc[i][0] for i in range(len(M_loc))],[M_loc[i][1] for i in range(len(M_loc))],
    #                        [M_loc[i][2] for i in range(len(M_loc))])
        legends=[f" drone {i} " for i in range(len(M_set_split))]
        for i in range(len(M_set_split)):
           #print(i)
            ax.scatter([self.dic_M_W.get(i)[0] for i in M_set_split[i]],[self.dic_M_W.get(i)[1] for i in M_set_split[i]],[self.dic_M_W.get(i)[2] for i in M_set_split[i]],color=co[i])
        #ax.scatter([self.dic_M_W.get(i)[0] for i in Center],[self.dic_M_W.get(i)[1] for i in Center],[self.dic_M_W.get(i)[2] for i in Center],color='r',s=50)
        ...
        plt.savefig(filename,bbox_inches = 'tight')
        plt.show()               
    def do_improve(self,max_time=0,max_inter=0):
        t_start=time.time()
        #print(f"improve start at", t_start)
        ...
        num_iter=0
        def stop_condition():
            #print(f"why it will return {time.time()-t_start}")
            if max_time==0 and max_inter==0:
                return num_iter>self.Max_iter or time.time()-t_start>self.Max_time or no_improve>=4
            elif max_time!=0 and max_inter==0:
                return num_iter>self.Max_iter or time.time()-t_start>max_time or no_improve>=4
            elif max_time==0 and max_inter!=0:
                return num_iter>max_inter or time.time()-t_start>self.Max_time or no_improve>=4
            else:
                return num_iter>max_inter or time.time()-t_start>max_time or no_improve>=4
        while not stop_condition():
            #print(f"---------- iteration {num_iter}---------\n ")
            #print(f"at duration {time.time()-t_start}" )
            Sum_t=sum(Best_p.to_t)
            Max_t=max(Best_p.to_t)
            Max_t_2,poor=sorted((Best_p.to_t[i],i) for i in range(len(M_set_split)))[-1]
            #print(f"check if they are same {Max_t},{Max_t_2}")
    ...
    #         while not stop:
           ...
           # print(f"where is leaf {leafs}, {agents_re}")
            check=30;seq=0;breaker=False
            va=random.random()
            if va<=0.99:
                #net=-1000
                c_time=time.time()
                area=list(range(len(Best_p.to_t)))
                area.remove(poor)
                random.shuffle(area)
                area2=[poor]+area
                #print(f"math time {time.time()-c_time}")
                #print(f"see {poor} {area2}")
                for i in area2:
                    to_sort=[ii for ii in range(len(Best_p.to_t)) if ii!=i]
                    random.shuffle(to_sort)
                    #print(f"see if shuffle",to_sort)
                    for j in to_sort:
                    #for j in range(i+1,len(Best_p.to_t)):
                        #print(f"see if j can get to what {i} {j}")
    ...
    #                 for j in agents_re:
                        ...
                        #    print(f"Best_p {Best_p[0].id_set} {j} {i}")
                            ...
                            #print(f" transfer leaf {i} from {poor} to {j} is {max_tt}")
                           # print([i.to_t for i in new_agent])
                             #   max([a1.to_t,a2.to_t]+[Best_p[a].to_t for a in list(set(agents_re)-set([j]))]) sum_tt<=sum_t*1.005
#                             if max_tt<max_t: 
#                                 print(f" see transfer leaf {a} from {i} to {j} is {max_tt} {(max_t-max_tt)-(sum_tt-sum_t)}")
                            if max_tt<max_t: #and  sum_tt<=sum_t*1.005 :#and (max_t-max_tt)-(sum_tt-sum_t)>net:
                                #net=(max_t-max_tt)-(sum_tt-sum_t)
                                max_t=max_tt
                                sum_t=sum_tt
                                best_neigh=([i],j,new_group)
                                #print(f" transfer leaf {a} from {i} to {j} is {max_tt}")
                            else:
                                if max_tt==max_t and sum_tt<sum_t:# and (max_t-max_tt)-(sum_tt-sum_t)>net:
                                    sum_t=sum_tt
                                    #max_t=max_tt
                                    #net=(max_t-max_tt)-(sum_tt-sum_t)
                                    best_neigh=([i],j,new_group)
                                    #print(f" sum changed!: transfer leaf {a} from {i} to {j} is {max_tt} {sum_t}")
#                                 else:
#                                     print(f" did not leaf {a} from {i} to {j} is {max_tt},{sum_tt}")
                            seq=seq+1      
                            if seq>=check:
                                if best_neigh==[] and check<=300:
                                    check=check+30
                                else:
                                    breaker=True
                                    break
            ...
            if best_neigh==[] and not stop_condition() and no_improve>=4:
                #print(f"get into process other swap {va}")
...
#                                     else:
#                                         print(f"did not work!: swap leaf {a}, {b} from {i} to {j} is {max_tt} {sum_tt}")
                                seq=seq+1
                                if seq>=check:
                                    if best_neigh==[] and check<=1500:
                                        check=check+200
...
#                                         print(f" sum changed!: swap leaf {a}, {b} from {i} to {j} is {max_tt}")
            ...
            if best_neigh!=[]: 
                ...
                #break
#             if num_iter%10==0:
#                 filename=f"../drone_matlab/best_{num_iter}.eps"
#                 self.draw(Best_p.M_set_split,Best_p.Center,filename)
#                 print(Best_p.M_set_split,Best_p.Center)
        self.Groups=Best_p
        #print(f"end at time { time.time()-t_start} {time.time()}")
        return Best_p
