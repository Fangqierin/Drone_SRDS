...
#                 f=open(f"./result/12_result/new2_{drone_num}_{0}_{events}_{th}.txt",'r')
            if th==0:
                f=open(f"./result/12_result/new1_{drone_num}_{1}_{events}_{1}.txt",'r')
            else:
...
#              'Minimum sampling accuracy of high-priority targets', 'Minimum sampling accuracy', 'Minimum detection ratio',  'Minimum detection ratio of high-priority targets ',  ' Improved sum of weighted sampling accuracy', ' Improved sum of weighted detection ratio','Running time (s)'  ]   
        ...
        if p==0:
            ...
            legends=[methods[i]+'$_{L}$' for i in show]
...
#             #filename=(f"./result/24_result/{par}_{name[pa]}_{drone_num}_{p}_{events}_{th}.eps")
...
for pa in [1,-3,-2,-1]: #range(len(parp)):#  #  
    ...
    legends3=[methods[i]+'$_{A}$'+', $M_{u}=$'f"{k}s" for k in [0,30,60]]
    #plt.yscale('symlog')
    legends3=legends3+[methods[i]+'$_{W}$']
    hfont = {'fontname':'Numbers','size':'14'}
    plt.xlabel('Plan Duration (Minute)',**hfont)
    plt.ylabel(title[pa],**hfont)
    #plt.yticks(np.arange(0,max([parp[p][j][i]/T[j] for j in range(len(T))])+0.1 , step=0.1))
    plt.grid( linestyle='--', linewidth=1)
    plt.legend(legends3,loc=4,prop={'size': 13},ncol=2)
    par='time'
