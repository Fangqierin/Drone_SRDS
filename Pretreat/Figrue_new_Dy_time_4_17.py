...
# for i in range(len(T)):
    ...
    for size in [2]:
        ...
        for p in [0]:
            #if p==2:
            #dy2022_14_5_2_2_0
            f=open(f"./result/result_2021/dy2022_15_{run_time}_{drone_num}_{size}_{policy}_{p}.txt",'r')
#             if drone_num==7:
#                 if size==2:
...
for pa in range(len(Total_st[ke])): 
    ...
    plt.xticks(range(len(h[show[0]])),[5,10,15,20,30])
    plt.grid( linestyle='--', linewidth=1)
    plt.ylabel(metrics2[cc])
