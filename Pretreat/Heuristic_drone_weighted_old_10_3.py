...
import random 
import time
from Integral import Integral_grain, Sum_Integral, Sump_Integral,Reliable,Waiting_grain
######################################################
...
def Get_Re_Min(case,wap,m_set,cu_t,D,w_id,task_dic,ins_m_dic,T_tm,co_s=1):
    #print(f"to check this one ooooo! {co_s} {case} ")
    ...
    if case==7 or case==8 or case==9 or case==10:
        ...
        for m in m_set.values():
            ...
            tmp_ac=m.ac_re
            #print(tmp_ac,si)
            #print([s*co_s if s>1 else s for s in si ])
