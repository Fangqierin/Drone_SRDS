...
if c_t==3:
    task_dic={'df':[1.2,0.003],'ds':[1,0.003],'dh':[1,0.003],'dw':[1,0.001],'mf':[3,0.015],'mh':[3,0.01],'dw2':[3,0.008],'df2':[3,0.01],'dh2':[3,0.01],'mw':[3,0.005]}
if c_t==2:
...
#plt.savefig(f"./result/partition_{self.Drone_num}_{self.T}.eps",bbox_inches = 'tight')
    ...
    def M_partition(self,par_case): 
        ...
        if self.Drone_num==1:
            ...
            partition=Partitioner(groups,M_dis,M_fre,len(Center),dic_M_W,To,90,30)
            #print(f"groups ",groups.M_set_split) 
            if par_case!=2:
                groups=partition.do_improve()
            #self.draw(groups.M_set_split, groups.Center,[self.report,list(self.Task['mh'])],f"../drone_matlab/partition_output_{self.Drone_num}_{self.T}.txt")
