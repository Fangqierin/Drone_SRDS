...
plan_duration=300 
#T_t=[60,180,240,360,480,540,600,900,1200,1500,1800]#,2100,2400,2700]
...
Outputfile=f"result/result_2021/{file_name}_{Drone_Num}_{va}_{policy}_{par}.txt"
Drone_N=3
D=[5,15]    
Record_flag=True
faces=7
...
ins_m_dic={'15':{'df2':0.7,'df3':0.7,'df':0.7,'ds':0.9,'dh2':0.69,'dh':0.69,'dw':0,'dw2':0,'dw3':0,'mf':0,'mh':0,'ms':0,'mw':0},'10':{'df':0.7,'df3':0.7,'ds':0.8,'dh':0.9,'dw':0.8,'mf':0.7,'mh':0.5,'ms':0.7},
           '5':{'df':0.99,'df2':0.99,'df3':0.99,'ds':1,'dh':0.98,'dh2':0.98,'dw':0.8,'dw2':0.8,'dw3':0.8,'mf':0.89,'mh':0.9,'ms':1,'mw':0.80},'0':{'df2':0,'df':0,'df3':0,'dh2':0,'dw3':0,'dh':0,'ds':0,'dw2':0,'dw':0,'mf':0,'mh':0,'ms':0,'mw':0}} 
...
task_dic={'df':[1,0.002],'df3':[2,0.002],'ds':[1,0.001],'dh':[1,0.002],'dw':[1.5,0.002],'mf':[3,0.007],'mh':[3,0.007],'dw2':[3,0.007],'dw3':[1,0.008],'df2':[4,0.007],'dh2':[3,0.006],'mw':[3,0.005]}
...
class Controller: 
    def __init__(self,all_num,lay_num,Wap_ty_set=[],report=[],task_dic={},ins_m_dic={},Dis=[],f_num=2,f_up=1,p_floor=3,w_num=32,To=To,T=0,T_total=0,Drone_num=2,Dic_M_W=dic_M_W,all_floors=-1,all_windows=[]):
        ...
        if first:
            ...
            for i in self.all_windows:
                ...
                if i>=0 and i<all_num//lay_num:
                    n=n+self.get_neigh(1+i*lay_num,self.lay_num,self.lay_num)
            self.Task['dw'].update(n)
        else:
            self.Task['dw']=set()
...
#                 self.Task['dw3'].update(list(set(n)-old_win))
    ...
    ################### dh I want add more 
        ...
        if not first: 
            ...
            for i in list(self.Table['fire']):
                n2=self.get_neigh(i,self.lay_num,2)
                for  j in range(2):
                    if i//self.lay_num<(layers-1-j):
                        n2=n2+self.get_neigh(i+(j+1)*self.lay_num,lay_num,2)
        ...
        if not first:
           ...
           # print(f"see fire_floors {fire_floors}")
            for i in list(self.Table['human']):
                if i//self.lay_num in fire_floors:
                    self.Task['mh'].update([i])
            #print(f"see task {self.Task}")
    #             else:
    #                 self.Task['mh'].update(i)
    ...
    def M_partition(self): 
        ...
        if self.Drone_num==1:
            ...
            self.partition=Partitioner(groups,M_dis,M_fre,len(Center),dic_M_W,To,40,20,0)
            if par!=2:
    ...
    def Update_partition(self,add_M,remove_M,update_M,la_w,enter_monitor):
        ...
        if self.Drone_num==1:
            ...
            if par!=2 and enter_monitor:
                ...
                groups=self.partition.do_improve(max_time=10)
                self.M_set_split=groups.M_set_split
                self.group=groups
...
#                 print(f"see check {tk}", check_fire)
        ...
        for tt in timeline:
            if tt>check_t:# check update to decide if to re-plan the motion. 
                ...
                if Update==True and Replan==True:
                    ww=Where(tt)
            ...
            for w in P_com.get(tt):
                ...
                #see.append([n for n in cov if n in fire_state])
                #see.append([n for n in cov if n in human_state])
                for tk in tasks:
       ...
       # print(f"see what is your perception", self.perception)
        Update,Replan,Table2,enter_monitor=check_update()
        ww=Where(tt)
        #print(f"see  conlutions if has h_s {conclusion}")
        #print(f"see state include state {self.Record_state}")
        #print(f" perception at {tt} {Update} {Table2}")
        if (tt-start_time+1)*60>=c.T_total:
            return Update,Replan,Table2, tt-start_time, ww, False,True
        else:
            #print(f"want to see time {(tt-start_time)*60} {c.T_total}")
            return Update,Replan,Table2, tt-start_time, ww, False,False
...
def Get_sim(rseed,a,output,time_slot):
    ...
    all_floors=set([i//lay_num for i in all_fire])
    all_windows=set([i//lay_num for i in all_fire])#]+[i//lay_num -1 for i in all_fire])
    ...
    for index,row in Timeline.iterrows():
        ...
        if len(tmp)==1 and tmp[0]=='':
            ...
            fire_floors=list(set(fire_floors+[fire[i]//lay_num+1 for i in range(len(fire))]+[fire[i]//lay_num+1 for i in range(len(fire))]+
                                 [fire[i]//lay_num-1 for i in range(len(fire))])) #here should add 2 upper layer, and one lower layer.
#             if fire[i]//lay_num>1:
...
#                     win.append(i)
        ...
        try:
            ...
            Sim_real[row['t']]['h_s']=[]
            Sim_real[row['t']]['h']=[]     
    return Sim,Sim_real,all_floors, all_windows


def check_perception(Sim,R_Table,start_sim,Total,T_time,outlog,i,method,ii,R_state):
...
#                     print(f" miss window at {tt} {len(set(sim)-set(see))} see {set(see)} with sim {set(sim)}")
    ...
    for t,state in R_state.items():
        ...
        for s, j in state.items(): # for f_s and h_s
            tmp=[]
            com_tmp=[]
            fire_floors=set([i//lay_num for i in Sim[t].get('f')]+[i//lay_num+1 for i in Sim[t].get('f')])
            state=dict(zip(Sim[t].get(s[0]),Sim[t].get(s)))
            for i in Sim[t].get(s[0]):
                if s[0]=='h' and i//lay_num not in fire_floors:
                    pass
                else:
...
#         print(f"{T_time[i]} {miss_state}")
        try:
            outlog.write(f"sum_state {i} {j} {ii} {T_time[i]} {miss_list[i]+miss_state[i]} \n")
        except:
...
while ii<10:
    ...
    Sim,Sim_real,all_floors,all_windows=Get_sim(rseed,a,output,time_slot)
...
#     print(f"fire source" ,fire_source)
    for T_total in [simulation_time]:
        for t_f in [plan_duration]: 
            c_orgin=Controller(all_num,lay_num,Wap_ty,fire_source,task_dic,ins_m_dic,Dis,Drone_num=Drone_Num,T_total=T_total,all_floors=all_floors,all_windows=all_windows)   
            c_orgin.First_task()
...
#                         #######################
                    ...
                    miss,miss_state=check_perception(Sim,c.Record_table,start_sim,c.T_total,T_t,outlog,i,j,ii,c.Record_state)
                   # print(f"see task",c.Task)
                    ...
                    print(f"see the result {drone.id} {i} {j} {miss} {miss_state}")
                    ...
                    outlog.write(f"AUC {i} {j} {ii} {w_min_v} {w_sample_min} {w_detect_min} {av_w_sample_min} {w_detect_min}\n")
                    outlog.write(f" \n")
