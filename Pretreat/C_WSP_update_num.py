...
ins_m_dic={'15':{'df2':0.7,'df':0.7,'ds':0.9,'dh2':0,'dh':0.7,'dw':0,'dw2':0,'dw3':0,'mf':0,'mh':0.7,'ms':0.9,'mw':0},'10':{'df':0.7,'ds':0.8,'dh':0.9,'dw':0.8,'mf':0.7,'mh':0.5,'ms':0.7},
           '5':{'df':1,'df2':1,'ds':1,'dh':1,'dh2':1,'dw':1,'dw2':1,'dw3':1,'mf':1,'mh':1,'ms':1,'mw':1},'0':{'df2':0,'df':0,'dh2':0,'dw3':0,'dh':0,'ds':0,'dw2':0,'dw':0,'mf':0,'mh':0,'ms':1,'mw':0}} 
...
task_dic={'df':[1,0.002],'ds':[1,0.001],'dh':[1,0.002],'dw':[1,0],'mf':[3,0.005],'mh':[3,0.005],'dw2':[3,0.008],'dw3':[3,0],'df2':[3,0.008],'dh2':[3,0.005],'mw':[3,0.005]}
...
class Controller: 
    ...
    def Gen_Task(self): # Define all tasks according to 'plicy', report and collected data. 
       ...
       # print(f"want to check", self.report, self.Task['dw'])
        ...
        print(self.Task)
        self.Task['dw2'].update(list(self.Table['fire']-self.Table['window']))
    ...
    def M_partition(self): 
        ...
        if self.Drone_num==1:
            ...
            self.partition=Partitioner(groups,M_dis,M_fre,len(Center),dic_M_W,To,120,80,0)
            if par!=2:
    ...
    def Update_Ma(self,at_t,la_w):
        ...
        if old_IM!=self.IM_set:
            self.Update_partition(add_M, remove_M, list(set(update_M)),la_w)
        
    def Update_partition(self,add_M,remove_M,update_M,la_w):
    ...
    def Update_Task(self,update,Replan,P_list,ww,at_t,Table,Wap_dic):   
...
#                 print(f"see self perception",self.perception)
            return Update, Replan, Table2
        def Where(tt):
            where=[]
...
#                 print(f"see check {tk}", check_fire)
                ...
                for s in check_area: # for check fire
                    for tk in tasks:
                        ...
                        if self.perception[tk].get(s)!=None:
                            self.perception[tk][s]=self.perception[tk][s]+acc
       ...
       # print(f"see what is your perception", self.perception)
        ...
        if (tt-start_time+1)*60>=c.T_total:
            return Update,Replan,Table2, tt-start_time, ww, True
        else:
...
def check_perception(Sim,R_Table,start_sim,Total,T_time,outlog,i,j,ii):
...
T_t=[60,180,240,360,540,600,900,1200,1500,1800,2100,2400,2700]
time_slot=1
ii=0
start_sim=20
while ii<20:
    ...
    Sim,Sim_real=Get_sim(rseed,a,output,time_slot)
...
#     print(f"fire source" ,fire_source)
    for T_total in [max(T_t)]:
        for t_f in [600]: 
            c_orgin=Controller(all_num,lay_num,Wap_ty,fire_source,task_dic,ins_m_dic,Dis,Drone_num=Drone_Num,T_total=T_total)   
            c_orgin.First_task()
            ...
            for i in range(-1,0):
                for j in range(14):  
                    ...
                    while done==False:
                        ...
                        t_l=time.time()
                        update,Replan,Table,cu_time,ww,done=c.track(Sim_real,P_list,time_slot,logfile,1,Wap_dic,start_sim) 
                        c.Update_Task(update,Replan,P_list,ww,cu_time,Table,Wap_dic)  # update task.update Ma, cu_time. 
...
#                         #######################
                    ...
                    miss=check_perception(Sim,c.Record_table,start_sim,c.T_total,T_t,outlog,i,j,ii)
                   # print(f"see task",c.Task)
                    ...
                    #print(f"see the result {drone.id} {i} {j} {miss}\n")
