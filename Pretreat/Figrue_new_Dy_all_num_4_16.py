...
#             Par_error2[drone_num]=std2
            Total_st[drone_num][p]=total_m
            #Total_st[size][p]=total_m
...
for pa in range(len(Total_st[ke][0])):    # for different metrics. 
    #m_n=0
    for pp in [2]:#list(Total_st[ke].keys()):  # all allocation method. 
        for i in [show1[-1]]:
            h=defaultdict(list);err=defaultdict(list)
            for k in list(Total_st.keys()): # [1,2,3,4]
                me=Total_st[k][pp][pa]
                #print(f"see me",me)
                for num in me[i]:
                    #print(f"num",i,num,np.mean(me[i][num]),me[i][num])
...
#                 h[i].append(a)
#                 err[i].append(er)
            #legends2.append(al[pp]+methods[i])
            #print(i,pp,h[i])
            #plt.errorbar(range(len(h[i])),h[i],yerr=err[i],fmt=f"{mark[m_n]}",capsize=2,markersize=7 )
            
            #m_n=m_n+1
    #plt.xticks(range(len(h[show1[-1]])),[3,5,7,9])
#for pa in range(len(Total_st[ke][0])):    # for different metrics. 
    m_n=0
    for pp in list(Total_st[ke].keys()):  # all allocation method. 
        for i in show1:
            h=defaultdict(list);err=defaultdict(list)
            for k in list(Total_st.keys()): # [1,2,3,4]
                me=Total_st[k][pp][pa]
                ...
                for num in me[i]:
                    ...
                    me[i][num]=np.mean(me[i][num])
                tmp=list(me[i].values())
                tmp2=[tmp[c]/last_result[k][c] for c in range(len(tmp))]
                a=np.mean(tmp2)
                sd=np.std(tmp2)   # get the std 
                er=conf_co*(sd/math.sqrt(len(tmp2)))
                h[i].append(a)
                err[i].append(er)
            legends2.append(al[pp]+methods[i])
#             print(i,pp,h[i])
            ...
            m_n=m_n+1
    plt.xticks(range(len(h[show1[-1]])),[3,5,7,9])
    #plt.xticks(range(len(h[show1[-1]])),[96,177,250,314])
    plt.grid( linestyle='--', linewidth=1)
    plt.ylabel(f"Normalized {metrics2[cc]}")
    #plt.xlabel('Number of Tasks')
    plt.xlabel('Number of Drones')
    plt.legend(legends2,prop={'size': 13},ncol=2)
    filename=(f"./result/2022_result/{par}_nom_num_{names2[cc]}_{drone_num}.eps")
    #filename=(f"./result/2022_result/{par}_nom_tasks_{names2[cc]}_{drone_num}.eps")
