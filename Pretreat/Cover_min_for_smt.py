...
#plt.savefig(f"./result/partition_{self.Drone_num}_{self.T}.eps",bbox_inches = 'tight')
    ...
    def M_partition(self,par_case): 
        ...
        if self.Drone_num==1:
            ...
            if par_case!=2 and flag_improve==True:
...
class Drone:     # WPC   # All Monitoring areas with tasks
    ...
    def Do_Heuristic(self,kind,case,co_s):  
        ...
        if case==11:
            t=time.time()
            P,cu_t=Greedy_SMT(kind,Wap_set,self.Ma_set,self.Dis,self.task_dic, self.ins_m_dic,self.cu_wp,self.cu_time,min(self.cu_time+self.T,self.T_total))
            tt=time.time()-t
        else:
            ...
            t=time.time()
            P,cu_t=Greedy_Min(case,co_s,Wap_set,self.Ma_set,self.Dis,self.task_dic, self.ins_m_dic,self.cu_wp,self.cu_time,min(self.cu_time+self.T,self.T_total),self.T_total)
            tt=time.time()-t
#         if case==5: 
