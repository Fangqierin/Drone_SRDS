...
class Controller:
    ...
    def Update_Ma(self, at_t, la_w, enter_monitor, par=0):
        tmp = [list(list(self.Task.values())[k]) for k in range(len(self.Task.values()))]
        old_IM = self.IM_set
        self.IM_set = list(set(itertools.chain.from_iterable(tmp)))
        Ma_new = {};
    ...
    def track(self, Sim, P_list, sim_logging_file, check_slot, Wap_dic, start_time, cu_time):
        ...
        Update, Replan, Table2, enter_monitor = check_update()
        ww = Where(tt)
        if (tt - start_time + 1) * 60 >= self.T_total:
...
def run_sim(Drone_Num, plan_duration, simulation_time, R_v, To, sim_seed=0, fire_number=3):
    ...
    va = fire_number  ### Number of fire sources
