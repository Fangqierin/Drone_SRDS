...
PATH_TO_TEST_IMAGES_DIR = pathlib.Path('./images/close_result/')

output_path='./images/close_result3'
TEST_IMAGE_PATHS = sorted(list(PATH_TO_TEST_IMAGES_DIR.glob("*.jp*")))
names=[str(i).split('/')[-1] for i in TEST_IMAGE_PATHS]
result_names=[f"result_{i}" for i in names]
PATH_TO_LABELS = 'object_detection/data/mscoco_label_map.pbtxt'
category_index = label_map_util.create_category_index_from_labelmap(PATH_TO_LABELS, use_display_name=True)
print(f"see category_index {category_index}")
for image_path in TEST_IMAGE_PATHS:
    show_inference(masking_model, image_path,output_path)

# Paths_to_dir=pathlib.Path('./images/far')
# Test_image_paths=sorted(list(Paths_to_dir.glob("*.j*")))
# output_path="images/far_result/"
